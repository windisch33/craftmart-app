...
type Props = {
  ...
  onOpenInvoicesForCustomer: (row: SalesGroupRow) => Promise<void> | void;
};

const SalesByCustomerSection: React.FC<Props> = ({ rows, onDownloadCsv, onDownloadPdf, onOpenInvoicesForCustomer }) => {
  ...
  return (
    <div style={{...cardStyle, marginBottom: '32px'}}>
      <h3 style={{fontSize: '20px', fontWeight: 'bold', color: '#1f2937', marginBottom: '16px'}}>Sales by Customer</h3>
      ...
      <div style={{overflowX:'auto'}}>
        <table style={{width:'100%', borderCollapse:'collapse'}}>
          <thead>
            <tr>
              {['Customer','Invoices','Subtotal','Tax','Total','Avg Invoice','Last Invoice Date','Actions'].map(h => (
          ...
          <tbody>
            {rows.map((r, i) => (
              <tr key={i} style={{cursor:'pointer'}} onClick={() => onOpenInvoicesForCustomer(r)}>
                ...
                <td style={{padding:'8px', borderBottom:'1px solid #f3f4f6'}}>
                  <button className="export-btn" onClick={(e)=>{ e.stopPropagation(); onOpenInvoicesForCustomer(r); }}>View Invoices</button>
                </td>
              </tr>
            ))}
          </tbody>
...
export default SalesByCustomerSection;
